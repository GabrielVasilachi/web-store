...
export function ProductCard({
  ...
  return (
    ...
    >
      ...
      >
        <span className="product-badge mono">
          {product.badge ?? 'EVERYDAY ESSENTIAL'}
        </span>
        <button
        ...
        <span className="product-background-word" aria-hidden="true">
          {product.category === 'Motorsport'
            ? 'FULL SEND.'
            : product.category === 'Color'
              ? 'RED FLAG.'
              : product.id === 'sunny-motel'
                ? 'OFF DUTY.'
                : 'AFTER DARK.'}
        </span>
        <motion.img
        ...
        />
        <span className="product-visual__caption mono">
          FH / 001 — HEAVY COTTON
        </span>
      </div>
      <div className="product-info">
        <div>
          <p className="mono">{product.subtitle}</p>
          <h3>{product.name}</h3>
        </div>
      ...
      <div className="product-meta">
        ...
        <span>Unisex / relaxed</span>
      </div>
      <div className="product-purchase">
        <div
          className="size-list"
          role="group"
          aria-label={`Mărime pentru ${product.name}`}
        >
        ...
        <button
          ...
          aria-label={`Adaugă ${product.name}, mărimea ${selectedSize}, în coș`}
        >
          <PlusIcon />
          <span>Adaugă</span>
        </button>
